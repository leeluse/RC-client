...
const Edit = () => {
  ...
  const [stateDropdown, setStateDropdowm] = useState(false);
  const periodDropdownHandler = () => {
    setPeriodDropdown(!periodDropdown);
  }

  const stateDropdownHandler = () => {
    setStateDropdowm(!stateDropdown);
  }
  ...
  const [productStatus, setProductStatus] = useState(status || '예약 가능');
            ...
            <div className='space-y-10 '>
              {imageData.thumbnail == null ? (
                <div className='flex justify-center items-center rounded-md w-40 h-40 lg:h-60 lg:w-60 border-2 shadow-xl border-slate-400'>
                  <MdAddPhotoAlternate onClick={clickHandler} className='text-slate-800 h-12 w-12 lg:h-20 cursor-pointer'/>
                </div>
              ) : (
              ...
              <input type="file" className='hidden' accept='image/*' onChange={fileUploader} ref={imageInput} />
            </div>
          </div>
          <div className='flex flex-col justify-center items-center'>
            <div className='space-y-5 lg:space-y-10 p-5 lg:p-10'>
              ...
              <div className='flex items-center'>
                ...
                <div
                  ...
                  <p>{period}</p>
                  {periodDropdown && (
                    <div className='rounded-md p-1 z-100 top-9 absolute border-2 bg-white text-slate-400 flex flex-col w-full'>
              ...
              <div className='flex items-center'>
                <span className='text-lg mr-2 font-bold text-slate-700'>상태</span>
                <div
                  onClick={stateDropdownHandler}
                  className='border flex justify-center cursor-pointer rounded-md px-4 py-1 bg-slate-400 text-white relative'>
                  <p>{productStatus}</p>
                  {stateDropdown && (
                    <div className='rounded-md p-1 top-9 absolute border-2 bg-white text-slate-400 flex flex-col w-full'>
                      <p onClick={() => statusHandler("예약 가능")}>예약 가능</p>
                      <p onClick={() => statusHandler("예약 중")}>예약 중</p>
                      <p onClick={() => statusHandler("렌탈 중")}>렌탈 중</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className=''>
              <button
                onClick={editHandler}
                className='border-2 mr-2 rounded-md text-slate-500 border-slate-500 py-2 px-8'>
                수정하기
              </button>
              <button
                onClick={deleteHandler}
                className='border-2 rounded-md text-slate-500 border-slate-500 py-2 px-8'>
                삭제
              </button>
            </div>
          </div>
